...
substr_conc <- c(0,5,6,7,8,10,12,15,20,40,80,160)
...
maxSubtrateConc <- 160 # maximum substrate concentration used
activeSites <- 4 # number of active sites of the enzyme
abs = 1.5 #targetted maximum absorbance
conc = 1.5/eps #Molar
conc = conc * 1000000 #microMolar
velocity = conc * cuv_vol #microMoles/min
velocity = velocity/60 #microMoles/s 

# Function to correct enzyme concentration to a normalized absorbence
enzymeConc_calc <- function(KM, Kcat){
  KM_S = ((maxSubtrateConc) + (KM))
  velocity = 1.5/eps #Molar/min
  velocity = velocity * 1000 #milliMolar/min
  velocity = velocity * (cuv_vol * 1000) #milliMoles/min
  velocity = velocity/60 #milliMoles/sec
  Vmax = (velocity * KM_S)/(maxSubtrateConc)
  moles_of_enzyme = Vmax/(activeSites * Kcat) #milliMoles 
  conc_of_enzyme = moles_of_enzyme / enz_vol #milliMolar
  return(conc_of_enzyme)
}

# Function to calculate new Vmax due to recalculated enzyme concentration
Vmax_calc <- function(Kcat, enzymeConc){
  moles_of_enzyme = enzymeConc * enz_vol
  Vmax = moles_of_enzyme * activeSites * Kcat #milliMolar/sec
  return(Vmax)
}

# Enzyme concentration calculation
enzymeConc <- rxn_params %>%
  filter(inhibition_actual == "no_inhibition") %>% 
  mutate(enzyme_conc = signif(enzymeConc_calc(Km, Kcat), digits = 2)) %>% 
  select(rxn_substrate, enzyme_conc)

# Ensure the same enzyme conc is used for the same substrate, Vmax calculated
rxn_params <- rxn_params %>% 
  select(-enzyme_conc) %>% 
  left_join(., enzymeConc) %>% 
  mutate(Vmax = Vmax_calc(Kcat, enzyme_conc))

# Correction of Vmax for uncompetitive or noncomoetitive inhibition
rxn_params_temp <- rxn_params %>%
  filter(inhibition_actual %in% c("uncompetitive", "noncompetitive")) %>% 
  mutate(Vmax = Vmax * 0.6)

rxn_params <- rxn_params %>%
  subset(!(inhibition_actual %in% c("uncompetitive", "noncompetitive"))) %>% 
  full_join(., rxn_params_temp) %>% 
  mutate(Vmax = Vmax * 60) #millimolar/min
...
estimated_Km_Vmax <- students_rxn_params %>% 
  ...
  mutate(rxn_rate = signif(rxn_rate, digits = 3)) %>% 
...
students_answers_data <- students_rxn_params %>% 
  ...
  unnest(data) %>% 
  select(student_id, rxn_substrate, inhibition_actual, abs_vs_time_plot, mm_plot, lb_plot, estimated_params, enzyme_conc) %>% 
  filter(inhibition_actual != "no_inhibition")
```
...
```{r}
createAnswerPDF <- function(student_id, rxn_substrate, inhibition_actual, abs_vs_time_plot, mm_plot, lb_plot, estimated_params, enzyme_conc){
  table1 <- tableGrob(tibble("info" = c("student", "substrate", "enzyme concentration"), " " = c(student_id, rxn_substrate, enzyme_conc)), rows = NULL, cols = NULL)
...
createAnswerPDF2 <- function(student_id, rxn_substrate, inhibition_actual, abs_vs_time_plot, mm_plot, lb_plot, estimated_params, enzyme_conc){
  table1 <- tableGrob(tibble("info" = c("student", "substrate", "enzyme concentration"), " " = c(student_id, rxn_substrate, enzyme_conc)), rows = NULL, cols = NULL)
  table2 <- tableGrob(unnest(estimated_params), rows = NULL)
  layout <- rbind(c(1,2), c(3,3), c(3,3), c(4,5), c(4,5))
  answers <- grid.arrange(table1, table2, abs_vs_time_plot, mm_plot, lb_plot, layout_matrix = layout)
